...
collection.delete_many({"last_active": {"$lt": dt.now(timezone.utc)}})
...
client = MongoClient('mongodb://localhost:27017')
db = client['history']  # 'history' est le nom de ta base de données
mongodb_collection = db['chathistory'] 
...
# Chain with History
    ...
    {"session_id": session_id},
    {
        ...
        "$set": {"last_active": dt.now(timezone.utc)}
    },
    upsert=True
)
